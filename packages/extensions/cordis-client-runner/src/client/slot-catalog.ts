...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  },
  {
    ...
    kind: 'list',
    scope: 'root',
    summary: 'Root-scope plugin primary content.',
    doc: 'Root-scope plugin primary content. The layout selects one registered id\nat a time and places it beside the shipped native conversation.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
  ...
  },
  {
    ...
    occupants: [],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'shell.workArea.companionHeader\', () => ctx.slots.register(\n      { name: \'shell.workArea.companionHeader\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-layout/src/client/index.ts:88',
  },
  {
    key: 'shell.workArea.footer',
    kind: 'list',
    scope: 'root',
    summary: 'Bottom strip spanning the work-area main column and its conversation companion.',
    doc: 'Bottom strip spanning the work-area main column and its conversation\ncompanion. A generic seat: an entry uses its work area\'s id, and the\nframe renders only the active id while a work area is open — with the\ncompanion hidden included, since the strip belongs to the work area,\nnot the conversation. It ships no built-in content. Absent entries\nrender nothing (upstream-identical).',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    occupants: [],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'shell.workArea.footer\', () => ctx.slots.register(\n      { name: \'shell.workArea.footer\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
