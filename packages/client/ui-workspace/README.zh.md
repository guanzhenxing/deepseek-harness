...
侧边栏区头还拥有一枚持久化的 Session 浏览器 disclosure 控件。它有可访问的展开状态，只控制列表区域：收起时保留区头、搜索、Workspace 操作以及全部 Session 数据，同时卸载分组或扁平的行树；展开或打开搜索时恢复这些行。它与组级展开独立，因此重新打开浏览器后仍会回到相同的 Workspace 与 Session 行状态。
