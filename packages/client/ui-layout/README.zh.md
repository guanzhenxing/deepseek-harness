...
- **工作区域选择是瞬时状态**：工作区域被选择期间必须保持其注册 id 存活；重新加载会回到普通外壳，隐藏的伴随栏重新挂载时会得到全新的本地 UI 状态。
